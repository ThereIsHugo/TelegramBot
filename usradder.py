...
def banner():
    ...
    print(f'{info}{g} TelegramBot [USERNAME] V1.0{rs}')
    print(f'{info}{g} Auteur : Hugo{rs}\n')
...
print(f'{info}{g} Ajout au groupe {group_name}{rs}\n')
n = 0
added_users = []
for user in users:
    n += 1
    ...
    if n % 50 == 0:
        print(f'{sleep}{g} Pause de 2 minutes{rs}')
        time.sleep(120)
    try:
        if user['username'] == "":
            continue
        ...
        usr_id = user['user_id']
        print(f'{attempt}{g} Ajout {usr_id}{rs}')
        print(f'{sleep}{g} Pause 30s{rs}')
        time.sleep(30)
        ...
        print(f'{error}{r} Restriction utilisateur{rs}')
        continue
    except KeyboardInterrupt:
        print(f'{error}{r} Abandon !{rs}')
        update_list(users, added_users)
        if not len(users) == 0:
            print(f'{info}{g} Utilisateurs restants connectés à {file}')
            logger = Relog(users, file)
            logger.start()
    except:
        print(f'{error}{r} Erreur dans l\'ajout{rs}')
        continue
#os.system(f'del {file}')
input(f'{info}{g}Ajout executé ! Appuyez sur [entrer] pour revenir')
sys.exit()
